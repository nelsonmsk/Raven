...
@section('body')
  ...
  <div class="col-lg-8 col-md-10 col-md-offset-2">
    ...
    <h2><span class="card-category">{{ __('Edit ProjectType') }} </span>
	<a href="{{config('app.url')}}/projectTypes" class="btn btn-secondary pull-right">View All</a></h2>
    </div>
	
<div class="card-body">	
    <form class="form-horizontal singleForm" id="projectTypes-form2" method="post" action="projectTypes" enctype="multipart/form-data" data-parsley-validate>
      <div class="form-group">
		 <input type="hidden" value="{{csrf_token()}}" id="_token" name="_token" />	  
         <input type="hidden" id="id" value="{{$projectType->id}}" name="id" />			   				
       <label for="name" class="col-sm-2 control-label">Name:</label>
		<div class="col-sm-10">
		  <input id="name" type="text"  class="form-control" name="name" value="{{$projectType->name}}" required />
		</div>
      </div>	  
