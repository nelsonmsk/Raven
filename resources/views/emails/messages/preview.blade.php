...
@section('body')
    ...
    <div class="col-md-10 col-md-offset-1 bg-white">
        <h3 class="text-center bold">Email Draft Preview </h3>
        <hr>
		<form class="form-horizontal singleForm" id="mailable-form2" method="post" action="emails" data-parsley-validate>
         ...
         <div class="p-top-30  panel" id="figures">
            <div class="container">
               <div class="row">
                  <div class="col-md-8 col-sm-9 center-block">
                     <div class="heading text-left">
                        <h3 class="light-font">Hi,  {{$messages->name ? $messages->name : '' }}</h3>
                        <p>{{$data['message']}}</p>
                        <p class="light-font">{{$messages->name ? 'Thank you for your message' : 'Thank you for your time. We would like to hear more feedback from you'}}</p>
                     </div>
					 <br/>					 
                  </div>
                  <!-- /.col --> 
               ...
               <div class="row">
                  <div class="col-md-8 col-sm-9 center-block">
                     <div class="heading text-left">
						...
						<br/>
							<p class="tr-dark ">{{Auth::user()->name}}</p>						
						<div><i class="light-font">							
							<p class="tr-dark">{{$appdefaults->companyName}}</p>
							<p class="tr-dark">{{$appdefaults->address}}</p>
							<a href="{{Auth::user()->email}}" class="tr-dark text-primary">{{Auth::user()->email}} </a>,
							<a href="{{$appdefaults->email}}" class="tr-dark text-primary">  {{$appdefaults->email}}</a><br>
							<a href="{{$appdefaults->phone}}" class="tr-dark text-danger">  {{$appdefaults->phone}}</a>							
							</i>						
						</div>
                     </div>					 
                  </div>
			...
			<div class="text-center">
				...
				<div id="b-element" class="form-group">
					<div class="col-sm-offset-2 col-sm-10">	
					  <button type="submit" class="btn btn-success btn-lg col-sm-5" id="save-btn" name="Send">Send <div class="fa fa-send text-white"></div></button>					
