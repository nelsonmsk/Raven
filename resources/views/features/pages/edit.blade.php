...
@section('body')
  ...
  <div class="col-lg-8 col-md-10 col-md-offset-2">
    ...
    <h2><span class="card-category">{{ __('Edit Feature') }} </span>
	<a href="{{config('app.url')}}/features" class="btn btn-secondary pull-right">View All</a></h2>
    </div>
	
<div class="card-body">	
    <form class="form-horizontal singleForm" id="features-form2" method="post" action="features" data-parsley-validate>
      <div class="form-group">
     <input type="hidden" value="{{csrf_token()}}" name="_token" />	  
           <input type="hidden" id="id" value="{{$feature->id}}" name="id" />			   
		    <input type="hidden" id="username" value="{{$feature->username}}" name="username" />			
      <label for="title" class="col-sm-2 control-label">{{ __('Title:') }}</label>
	<div class="col-sm-10">
      <input id="title" type="text"  class="form-control" name="title" value="{{$feature->title}}" required />
	</div>
      </div>
	<div class="form-group">
		<label class="col-sm-2 control-label">{{ __('Body:') }}</label>
		<div class="col-sm-10">
			  <textarea id="body" name="body" class="form-control">{{$feature->body}} </textarea>
		</div>
	</div>
    <div class="form-group">      
      <label for="icon" class="col-sm-2 control-label">{{ __('Icon:') }}</label>
		<div class="col-sm-10">
		  <input id="icon" type="text"  class="form-control" name="icon" value="{{$feature->icon}}" required />
		</div>
    </div>		
      <div class="form-group">
      <label for="pageId" class="col-sm-2 control-label">{{ __('Page Id:') }}</label>
	<div class="col-sm-10">
      <input id="pageId" type="text"  class="form-control" name="pageId" value="{{$feature->pageId}}" required />
	</div>
      </div>
